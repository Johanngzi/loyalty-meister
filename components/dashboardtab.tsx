...
export default function DashBoardTab({ className = "" }: DashBoardTabProps) {
  return (
    <div className={`flex w-full flex-col ${className}`}>
      {/* Title and Tabs (Now in Separate Containers) */}
      <div className="flex items-center gap-x-6">
        <h1 className="font-bold text-3xl pt-4 whitespace-nowrap">find a game</h1>
      </div>

      {/* Tabs - Ensure it's stacked correctly */}
      <Tabs
        aria-label="Options"
        classNames={{
          tabList:
            "gap-6 w-full relative rounded-none p-0 border-b border-divider",
          cursor: "w-full bg-[#bffb4f]",
          tab: "max-w-fit px-0 h-12",
          tabContent: "group-data-[selected=true]:text-[#bffb4f]",
        }}
        color="primary"
        variant="underlined"
      >
        ...
        >
          {/* Card content is now below the tabs */}
